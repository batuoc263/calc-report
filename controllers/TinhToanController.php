...
class TinhToanController extends \yii\web\Controller
{
    ...
    public function actionNoiSuy()
    {
        $dmtt = DmTinhtoan::findOne(['duong_dan' => '/tinh-toan/noi-suy']);
        $menu = DmNhomBai::find()->asArray()->all();
        foreach ($menu as $key => $value) {
            $menu[$key]['children'] = DmTinhtoan::find()->where(['nhom_id' => $value['id']])->asArray()->all();
        }
        ...
        if ($input = Yii::$app->request->post()) {
            ...
            $url = "https://isinhvien.com/dotools/?act=noisuy2c";
            

            $curl = curl_init();
            $data = [];
            foreach ($input['pp'] as $key => $value) {
                $data["pp[$key]"] = $value;
            }
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_HTTPHEADER => array(
                    'Cookie: PHPSESSID=3852178fa7a8d3914efee0b3c9ffd659; helpful_user=bc5cdcce3372c0eff345eba83f21ed54'
                ),
            ));
            ...
            echo json_encode(['luot_tinh' => $dmtt->luot_giai]);
            return;
        }

        return $this->render('noi-suy', [
            'dmtt' => $dmtt,
            'menu' => $menu,
        ]);
    }
